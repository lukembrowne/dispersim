...
for(step in 1:steps){
  ...
  if(step == 1 | (step %% (steps/save_status_freq) == 0)){
    ...
    # Slows down code quite a bit..
    if(save_status == TRUE){
#       ## Save species richness
#       ad_sp_over_time[n_iter, over_time] <- sum(length(table(ad_sp_id))) 
#       
#       ## Save avg genetic distance
#       ad_gen_over_time[n_iter, over_time] <- calc_mean_gen_dist(ad_sp_id = ad_sp_id,
#                                                                 ad_gen = ad_gen,
#                                                                 min_pop = min_pop,
#                                                                 gen_neighborhood = 10,
#                                                                 ad_x = ad_x, ad_y = ad_y,
